import {passwordInvalidSymbolsRegex, passwordMaxLen, passwordRegex, passwordStartsWithRegex} from "./regex";

export const isValidPassword = (password: string) => {
    if (!passwordStartsWithRegex.test(password)) throw new Error("Password should start with 3 normal letters");

    if (password.length > passwordMaxLen) throw new Error("Password should be in range of " + passwordMaxLen + " symbols");
